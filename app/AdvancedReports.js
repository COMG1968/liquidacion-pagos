...
export default function AdvancedReports(){
 ...
 useEffect(()=>{const check=()=>setShow([...document.querySelectorAll('h2')].some(h=>(h.textContent||'').includes('Informe de pagos')));check();const o=new MutationObserver(check);o.observe(document.body,{childList:true,subtree:true});return()=>o.disconnect()},[])
 useEffect(()=>{if(!show)return;(async()=>{const [{data:w},{data:m}]=await Promise.all([sb.from('trabajadores').select('id,nombre').order('nombre'),sb.from('metodos_pago').select('id,nombre').order('nombre')]);setWorkers(w||[]);setMethods(m||[])})()},[show])
 async function generate(){
  let q=sb.from('liquidaciones').select('*, trabajadores(nombre), metodos_pago(nombre)').order('fecha_desde',{ascending:false})
  if(from)q=q.gte('fecha_hasta',from);if(to)q=q.lte('fecha_desde',to);if(worker)q=q.eq('trabajador_id',Number(worker));if(status)q=q.eq('estado',status);if(method==='none')q=q.is('metodo_pago_id',null);else if(method)q=q.eq('metodo_pago_id',Number(method))
  const {data,error}=await q;if(error)return alert('No fue posible generar el informe: '+error.message)
  let out=data||[];if(min!=='')out=out.filter(x=>Number(x.gran_total??x.total_pago)>=Number(min));if(max!=='')out=out.filter(x=>Number(x.gran_total??x.total_pago)<=Number(max));setRows(out)
 }
 ...
 if(!show)return null
 return <section className="card advanced-reports" style={{marginTop:14}}>
  <h2>🔎 Informe avanzado por filtros</h2><p>Combina los filtros que necesites. Los que dejes en “Todos” no restringen el informe.</p>
