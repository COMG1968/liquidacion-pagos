...
export default function Page(){
 ...
 useEffect(()=>{let mounted=true;sb.auth.getSession().then(({data})=>{if(mounted){setSession(data.session);setAuthReady(true)}});const {data:{subscription}}=sb.auth.onAuthStateChange((_e,s)=>{setSession(s);setProfile(null);setAuthReady(true)});return()=>{mounted=false;subscription.unsubscribe()}},[])
 ...
 async function login(e){e?.preventDefault();let {error}=await sb.auth.signInWithPassword({email:email.trim(),password});if(error)return alert('No fue posible iniciar sesión: '+error.message);setPassword('')}
 ...
 async function save(mode='admin'){
  ...
  let payload={trabajador_id:Number(worker),fecha_desde:from,fecha_hasta:to,valor_hora:Number(rate),total_horas:totalHours,total_pago:total,observaciones:obs||null,estado:isWorker?'borrador':status,metodo_pago_id:isAdmin&&status==='pagado'?Number(method):null,gran_total:grandTotal}
  let id=editId
 ...
 function reset(){setWorker('');setFrom('');setTo('');setRows([]);setRate('');setObs('');setStatus('pendiente');setMethod('');setEditId(null);setAdjustments([])}
 async function editPay(p){let {data}=await sb.from('detalle_horas').select('*').eq('liquidacion_id',p.id);let {data:ad}=await sb.from('ajustes_liquidacion').select('*').eq('liquidacion_id',p.id).order('id');setAdjustments((ad||[]).map(a=>({tipo:a.tipo,concepto:a.concepto,valor:a.valor})));let old={};(data||[]).forEach(x=>old[x.fecha]=x.horas);setEditId(p.id);setWorker(String(p.trabajador_id));setRate(p.valor_hora);setFrom(p.fecha_desde);setTo(p.fecha_hasta);setObs(p.observaciones||'');setStatus(p.estado||'pendiente');setMethod(p.metodo_pago_id?String(p.metodo_pago_id):'');makeRows(p.fecha_desde,p.fecha_hasta,old);setTab('new')}
 ...
 <table><thead><tr><th>Trabajador</th><th>Período</th><th>Horas</th><th>Total</th><th>Estado / Pago</th><th>Acciones</th></tr></thead><tbody>{filtered.map(p=><tr key={p.id}><td>{p.trabajadores?.nombre}</td><td>{labelDate(p.fecha_desde)} / {labelDate(p.fecha_hasta)}</td><td>{p.total_horas}</td><td>{money(p.gran_total??p.total_pago)}</td><td>{p.estado||'pendiente'}{p.metodos_pago?.nombre?<><br/><small>{p.metodos_pago.nombre}</small></>:null}</td><td><div className="actions">{isAdmin?<><button onClick={()=>editPay(p)}>Editar</button><button onClick={()=>printPay(p)}>Imprimir</button>{['enviada','en_revision'].includes(p.estado)&&<><button className="success" onClick={()=>reviewPay(p,'aprobada')}>✓ Aprobar</button><button onClick={()=>reviewPay(p,'devuelta')}>↩ Devolver</button><button className="danger" onClick={()=>reviewPay(p,'rechazada')}>✕ Rechazar</button></>}<button className="danger" onClick={()=>delPay(p)}>Eliminar</button></>:<>{['borrador','devuelta'].includes(p.estado)&&<button onClick={()=>editPay(p)}>Editar</button>}<button onClick={()=>printPay(p)}>Ver / Imprimir</button></>}</div>{isAdmin&&['enviada','en_revision'].includes(p.estado)&&<textarea className="review-note" placeholder="Comentario para el trabajador (opcional)" value={reviewNote} onChange={e=>setReviewNote(e.target.value)}/>} {p.comentario_admin&&<small><b>Nota admin:</b> {p.comentario_admin}</small>}</td></tr>)}</tbody></table></section>}
